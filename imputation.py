...
class Imputation:
    ...
    tasks = [
        ...
        "6. Show the Dataset",
        "7. Show Changed Values",
        "8. Show Removed Columns",
    ]

    def __init__(self, data):
        self.data = data
        self.removed_columns = []  # List to store removed columns
        self.changed_values = {}
    ...
    def removeColumn(self):
        ...
        while(1):
            ...
            if choice=="y" or choice=="Y":
                ...
                # Check if columns exist in the data before removing them
                    for col in columns_list:
                        if col in self.data.columns:
                            self.removed_columns.append(col)  # Store removed column
                        else:
                    ...
                    print(f"{e}\nOne or more Columns are not present. Try again.....\U0001F974")
                continue
            else:
    ...
    def showChangedValues(self):
        if not self.changed_values:
            print("No changes have been made yet....")
        else:
            print("\nChanged Values:")
            for column, changes in self.changed_values.items():
                print(f"\nIn Column '{column}':")
                print(changes)
        return

    def showRemovedColumns(self):
        if not self.removed_columns:
            print("No columns have been removed yet....")
        else:
            print("\nRemoved Columns:")
            print(self.removed_columns)
        return
    ...
    def imputer(self):
        while(1):
            ...
            if choice == -1:
                ...
                self.showChangedValues()

            elif choice==8:
                self.showRemovedColumns()
